...
/* ---------- 场景 7：月度/年度报表 ---------- */
{
  ...
  /* 规格 §7：含浮动 = 快照差 − 当月净投入，**不再叠加 real**（v4 实现曾多加 real，属重复计算已实现） */
  eq('2月含浮动', feb.floatTotal, 12000 - 10000 - (-600), 0.01); // 净投入=卖出+600
  eq('2月总收益(已实现+浮动)', feb.total, 100 + (12000 - 10000 - (-600)), 0.01);
