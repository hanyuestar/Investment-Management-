<template>
  <div class="asset-card">
    ...
    <div class="ac-row"><span>数量/份额</span><b class="num">{{ h.calc.qty ? money(h.calc.qty, h.asset.type === 'stock' ? 0 : 2) : '—' }}</b></div>
    <div class="ac-row" v-if="h.asset.type === 'stock'">
      <span>成本价 / 现价 ({{ h.asset.currency }})</span>
      <b class="num">{{ money(h.calc.avgLocal, 3) }} / {{ money(h.asset.price, 3) }}</b>
    </div>
